...
The Creator of the script: @smsp4m
...
def requirements():
	...
	input_csv = input(gr+'['+cy+'+'+gr+']'+cy+' Вы хотите начать установку? (y/n): ').lower()
	if input_csv == "y":
		csv_lib()
	else:
		pass
	print(gr+"[!] Установка необходимых компонентов ... [!]")
	os.system("""
...
def config_setup():
	...
	apid = input(gr+"[+] Введите api ID : "+re)
	cpass.set('cred', 'id', apid)
	hashid = input(gr+"[+] Введите hash ID : "+re)
	cpass.set('cred', 'hash', hashid)
	phone_number = input(gr+"[+] Введите номер телефона : "+re)
...
def merge_csv():
	...
	print(gr+'['+cy+'+'+gr+']'+cy+' Большие файлы могут занять некоторое время ... ')
	merge = file1.merge(file2, on='username')
	merge.to_csv("output.csv", index=False)
	print(gr+'['+cy+'+'+gr+']'+cy+' Файл сохранён как "output.csv"\n')
...
def update_tool():
	...
	if source.text == '3':
		print(gr+'['+cy+'+'+gr+']'+cy+' Установлена последняя версия')
	else:
		print(gr+'['+cy+'+'+gr+']'+cy+' Удаление старых файлов ...')
		os.system('rm *.py');time.sleep(3)
		print(gr+'['+cy+'+'+gr+']'+cy+' Получение последних файлов ...')
		os.system("""
...
try:
	...
	print('\n'+gr+'['+re+'!'+gr+']'+cy+' Мало аргументов '+ sys.argv[1])
	print(gr+'['+re+'!'+gr+']'+cy+' Если проблема с установкой, то перейдите здесь: ')
